...
// showDeployments shows the GitHub environment deployment statuses for the repo,
// falling back to the dashboard for non-GitHub repos. The fetching and caching
// live in DeploymentsHelper, which the background refresh routine also drives.
func (self *StatusController) showDeployments() {
	if !self.c.Helpers().Host.HasGithubRemote() {
		// Keep the deployments cache keyed to the current repo even while we show
		// the dashboard, so returning to a GitHub repo refetches rather than
		// rendering another repo's stale deployments.
		self.c.Helpers().Deployments.InvalidateIfRepoChanged()
		self.showDashboard()
		return
	}

	self.c.Helpers().Deployments.Show()
}
